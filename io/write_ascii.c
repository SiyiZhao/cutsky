/*******************************************************************************
* write_ascii.c: this file is part of the cutsky program.
...
int output_writeline(OFILE *ofile, const char *restrict format, ...) {
  ...
  if (ofile->size == ofile->max) {
    if (output_flush(ofile)) return CUTSKY_ERR_FILE;
  }
  ...
  if (size < 0) {
    ...
    return CUTSKY_ERR_ASCII;
  }
  if (size >= CUTSKY_MAX_CHUNK) {
  ...
  if (size >= ofile->max - ofile->size) {
    ...
    while (size >= ofile->max) {
      if (CUTSKY_MAX_CHUNK / 2 < ofile->max) ofile->max = size + 1;
      else ofile->max <<= 1;
      enlarge = true;
    }
